...
def create_dataset_from_chunks(
    chunks_dir, output_dir, audio_column="audio", test_split=0.1, sample_rate=32000
):
    """
    ...
    for file_path in tqdm(audio_files, desc="Processing audio chunks"):
        ...
        # Add to data
        data.append(
            {"file_path": file_path, "file_name": file_name, audio_column: file_path}
        )
    ...
    dataset = DatasetDict.from_csv({"train": train_csv_path, "test": test_csv_path})
    ...
    print(
        f"You can load this dataset with: dataset = load_from_disk('{os.path.join(output_dir, 'dataset')}')"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create a HuggingFace dataset from existing audio chunks"
    )
    parser.add_argument(
        "--chunks-dir",
        "-c",
